...
func (rmo *pbResponseOrder) SendTo(p p2pcommon.RemotePeer) error {
	err := p.WriteMsg(rmo.message)
	if err != nil {
		rmo.logger.Warn().Str(p2putil.LogPeerName, p.Name()).Str(p2putil.LogProtoID, rmo.GetProtocolID().String()).Str(p2putil.LogMsgID, rmo.GetMsgID().String()).Err(err).Msg("fail to SendTo")
		return err
...
func (rmo *pbBlkNoticeOrder) SendTo(p p2pcommon.RemotePeer) error {
...
func (rmo *pbBpNoticeOrder) SendTo(p p2pcommon.RemotePeer) error {
...
func (rmo *pbTxNoticeOrder) SendTo(pi p2pcommon.RemotePeer) error {
	p := pi.(*remotePeerImpl)
...
func (rmo *pbRaftMsgOrder) SendTo(pi p2pcommon.RemotePeer) error {
	p := pi.(*remotePeerImpl)
...
func (rmo *pbTossOrder) SendTo(pi p2pcommon.RemotePeer) error {
	p := pi.(*remotePeerImpl)
	err := p.rw.WriteMsg(rmo.message)
	if err != nil {
