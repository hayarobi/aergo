...
// V230Handshaker exchange status data over protocol version 1.0.0
type V230Handshaker struct {
...
var _ p2pcommon.VersionedHandshaker = (*V230Handshaker)(nil)

func (h *V230Handshaker) GetMsgRW() p2pcommon.MsgReadWriter {
	return h.msgRW
}

func NewV230VersionedHS(is p2pcommon.InternalService, log *log.Logger, vm p2pcommon.VersionedManager, cm p2pcommon.CertificateManager, peerID types.PeerID, rwc io.ReadWriteCloser, genesis []byte) *V230Handshaker {
	h := &V230Handshaker{selfMeta: is.SelfMeta(), is: is, logger: log, peerID: peerID, localGenesisHash: genesis, vm: vm, cm: cm}
...
func (h *V230Handshaker) DoForOutbound(ctx context.Context) (*p2pcommon.HandshakeResult, error) {
	...
	// If consensus of node is raft (and remote peer is also raft because of same chainID),
	// try add getting another stream for raft
...
func (h *V230Handshaker) sendLocalStatus(ctx context.Context, hostStatus *types.Status) error {
	var err error
...
func (h *V230Handshaker) receiveRemoteStatus(ctx context.Context) (*types.Status, error) {
...
func (h *V230Handshaker) checkRemoteStatus(remotePeerStatus *types.Status) error {
...
func (h *V230Handshaker) DoForInbound(ctx context.Context) (*p2pcommon.HandshakeResult, error) {
	...
	// If consensus of node is raft (and remote peer is also raft because of same chainID),
	// prepare for getting another stream for raft before sending status
...
func (h *V230Handshaker) handleGoAway(peerID types.PeerID, data p2pcommon.Message) (*types.Status, error) {
...
func (h *V230Handshaker) sendGoAway(msg string) {
...
func (h *V230Handshaker) checkByRole(status *types.Status) error {
...
func (h *V230Handshaker) checkAgent(status *types.Status) error {
...
func (h *V230Handshaker) createLocalStatus(chainID *types.ChainID, bestBlock *types.Block) (*types.Status, error) {
